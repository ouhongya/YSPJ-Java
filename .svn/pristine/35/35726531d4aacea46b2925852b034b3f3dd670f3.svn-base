    ...
    public ResultModel<String> uploadExcel(HttpServletRequest res, MultipartFile fileStream, String uid) throws Exception {
        if (!fileStream.isEmpty()) {
            try {
                ...
                out.close();
                //解析Excel
                Object data = readData(new File(excelPath), suffix);
                if (data instanceof String) {
                    return ResultModel.failure(data.toString());
                }
                Map<String, LinkedList<Map<String, String>>> map = (Map<String, LinkedList<Map<String, String>>>) data;
                String user_id = uid;
                Integer function = (Integer) dao.findForObject("ExcelSettingMapper.queryUserByRoue", user_id);
                //存到数据库
                PageData excel = new PageData();
                Integer total_censor = 0;
                ...
                for (String key : map.keySet()) {
                    ...
                    for (Map<String, String> stringMap : mapList) {
                        ...
                        if (index >= 11) {
                            //检查项
                            if (!normDetail.get("content").equals("") && !normDetail.get("untitled").equals("")) {
                                total_censor += 1;
                            }
                ...
                excel.put("total_censor", total_censor);
                excel.put("status", 1);
                ...
                return ResultModel.success("解析失败请重新上传");
            }
